...
amplitude = (
    read_image("data/bars.png", mode=ImageReadMode.GRAY).squeeze(0).float() / 255.0
)
...
k_vectors: list[tuple[int, int]] = [
    (k[0], k[1]) for k in product(range(-20, 21, 20), repeat=2)
]
# 找到零频率（轴上）k-向量的索引
...
captures = forward_model(
    image_complex, pupil, kx_all, ky_all, downsample_factor=2
)  # 输出形状: [B, H, W]
...
plot_comparison(
    [amplitude.cpu(), captures[zero_idx].cpu(), pred_O_amplitude.cpu()],
    ["原始图像", "捕获图像（轴上）", "重建物体"],
    "tmp/adamw.png",
)
